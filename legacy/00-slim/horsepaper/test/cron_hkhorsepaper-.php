...
//-----------------------------------------		
        ...
        foreach($value as $key2 => $title):
			...
			$hl_smallerpla="";
			$TrumpCard="";
			$listr .= "<tr $td><td style=\"background-color:#FF99FF;border : 1px solid #FFFFFF;\">".$key2;	//horse no
/*same win-------------------*/
...
/*place pattern ----------------------------------------------------------------------------------------------------------*/					
			$listr .= "<td $td >".($hist_prop[$cnt][1] ? $hist_prop[$cnt][1] : "&nbsp;");	//($cnt==1  ? $hist_prop[1] : "&nbsp;");
			$listr .= "<td $td >".($hist_prop[$cnt][2] ? $hist_prop[$cnt][2] : "&nbsp;");	//($cnt==2  ? $hist_prop[2] : "&nbsp;");
			$listr .= "<td $td >".($hist_prop[$cnt][3] ? $hist_prop[$cnt][3] : "&nbsp;");	//($cnt==3  ? $hist_prop[3] : "&nbsp;");
			$listr .= "<td $td >".($hist_prop[$cnt][4] ? $hist_prop[$cnt][4] : "&nbsp;");	//($cnt==4  ? $hist_prop[4] : "&nbsp;");
			//$listr .= "<td >".($title['win']>0 ? number_format(1/($title['win']+$title['pla'])*0.82,2) : "&nbsp;");
/*race detail-------------------------------------------------------------------------------------------------------------*/
			$listr .= "<td $td align=left nowrap>".($runnerinfo[$key1][$key2]['Name']);
			$listr .= "<td $td align=left nowrap>".($racedetail[$key1][$key2]['HandicapWeight']);
			$listr .= "<td $td align=left nowrap>".($runnerinfo[$key1][$key2]['WeightAllowance'] ? $runnerinfo[$key1][$key2]['WeightAllowance'] : "&nbsp;");
			$listr .= "<td $td align=left nowrap>".($runnerinfo[$key1][$key2]['RatingRange'] ? $runnerinfo[$key1][$key2]['RatingRange'] : "&nbsp;");
			...
			$listr .= "<td $td align=left nowrap>".$TrumpCard.($runnerinfo[$key1][$key2]['Trainer'])."[".$remain_trainer[(string)$runnerinfo[$key1][$key2]['Trainer']].'/'.$TrainerCode_cnt[(string)$runnerinfo[$key1][$key2]['Trainer']]."]";
			$listr .= "<td $td align=left nowrap>".$resultcount_Trainer[(string)$runnerinfo[$key1][$key2]['Trainer']];
			//Jockey counter			
			...
			$listr .= "<td $td align=left nowrap>".($runnerinfo[$key1][$key2]['Jockey'])."[".$remain_jockey[(string)$runnerinfo[$key1][$key2]['Jockey']].'/'.$JockeyCode_cnt[(string)$runnerinfo[$key1][$key2]['Jockey']]."]";
			$listr .= "<td $td align=left nowrap>".$resultcount_Jockey[(string)$runnerinfo[$key1][$key2]['Jockey']];
			...
			$listr .= "<td $td align=left nowrap>"."[".$remain_trainerjockey[(string)$runnerinfo[$key1][$key2]['Trainer_Jockey']].'/'.$TrainerJockey_cnt[(string)$runnerinfo[$key1][$key2]['Trainer_Jockey']]."]";
			$listr .= "<td $td align=left nowrap>".$resultcount_TrainerJockey[(string)$runnerinfo[$key1][$key2]['Trainer'].(string)$runnerinfo[$key1][$key2]['Jockey']];
			//------------------------------------------------------------------------------------------------------------
			$listr .= "<td $td align=left nowrap>".$racedetail[$key1][$key2]['OddsInRange'];
			...
			$listr .= "<td $td nowrap>".$hl_samepla.$sel_pla.$hl_smallerpla.$title['pla'];
			$listr .= "<td $td nowrap>".(is_numeric($title['pla']) ? number_format(100*$title['pla']/$title['win'],0) : "&nbsp;");
			$bt_mini = ($bt_min==$runnerinfo[$key1][$key2]['BestTime_val']) ? "<font color=red>" : "";
			
			$listr .= "<td $td align=right nowrap>".$bt_mini.($runnerinfo[$key1][$key2]['BestTime'] ? $runnerinfo[$key1][$key2]['BestTime'] : "&nbsp;");
			$listr .= "<td $td align=right nowrap>".($runnerinfo[$key1][$key2]['StakesWon'] ? number_format($runnerinfo[$key1][$key2]['StakesWon']*1,0) : "&nbsp;");
			$listr .= "<td $td align=right nowrap>".($runnerinfo[$key1][$key2]['LastSixRuns']);
/*pick barrier result----------------------------------------------------------------------------------*/
...
if(mysqli_num_rows($search) ){
	...
	$row = mysqli_fetch_assoc($search);
		...
		$listr .= "<td $td nowrap>".$b.$row['barrierday'];
		$listr .= "<td $td nowrap>".$row['Going'];
		$samejokey = ($row['Jockey'] == $runnerinfo[$key1][$key2]['Jockey']) ? "<font color=red>" : ""; 
		$listr .= "<td $td nowrap>".$samejokey.$row['Jockey'];
		$listr .= "<td $td nowrap>".$row['Timex'];
		$listr .= "<td $td nowrap>".$row['Result'];
		$listr .= "<td $td nowrap>".$row['Comment'];
	// }
