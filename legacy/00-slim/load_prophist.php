...
function load_prophist($search_racingdate,$search_venue,$horsecnt,$TopPredigit,$propprecnt, $proppremorethanone,$if1x,$if2x,$if3x,$if4x,$if5x,$if6x, $unique1x, $unique2x, $unique3x, $unique4x, $unique5x, $unique6x){
    global $mysqli;
    ...
    // print_r($propprecnt);
    // print_r($proppremorethanone);
    $proppremorethanone = array_unique($proppremorethanone);
    foreach ($proppremorethanone as $morethanone) {
        $propprecntX = getFirstDigit($morethanone);
        $propXcount[$propprecntX."x"][] = $propprecnt[$propprecntX."_".$morethanone];
    }
    // print_r($propXcount);
    ...
    foreach ($prophist as $raceKey => $horses) {
        ...
        foreach ($horses as $horseno => $data) {
            ...
            if(is_numeric($horseno)){
                ...
                $prophist[$raceKey]['propprecnt'][$tensDigit."_".$proppreValue]++;
                
                if ($occurrences[$proppreValue]>1) {
                    $occurrencesx2['tensDigit'][] = $tensDigit;
                    $occurrencesx2[$tensDigit."x"][] = $proppreValue;
                    $prophist[$raceKey]['proppremorethanone'][] = $proppreValue;
    ...
    foreach ($prophist as $raceKey => $horses) {
        ...
        $histproppremorethanone = array_unique($prophist[$raceKey]['proppremorethanone']);
        // print_r($histproppremorethanone);
        foreach ($histproppremorethanone as $morethanone) {
            $histpropprecntX = getFirstDigit($morethanone);
            $histpropXcount[$raceKey][$histpropprecntX."x"][] = $prophist[$raceKey]['propprecnt'][$histpropprecntX."_".$morethanone];
        }
        if(compareArrays($propXcount, $histpropXcount[$raceKey]) ){
            // print_r($histpropXcount[$raceKey]);
            // print_r($prophist[$raceKey]);
            ...
            // $newProphist[$raceKey] = $prophist[$raceKey];
        }
    }
    return $newProphist;
