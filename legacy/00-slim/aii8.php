...
require_once 'PropFingerprint.php';
require_once 'PropManualRuleEngine.php';
...
// Manual rules for Sx column only (ST/HV stay on auto-trained finalpick3onlyrules)
$manualRulesSx = PropManualRuleEngine::loadEnabledRules($db, 'Sx');
$manualRulesSxN = count($manualRulesSx);
        ...
        foreach ($venuedetail as $venue => $racedetail) {
            ...
            foreach ($racedetail as $raceno => $race) {
                if (is_numeric($raceno) && is_array($race)){
                    ...
                    // Sx live column → enabled manual rules (compare vs ST/HV auto)
                    $preds_picks_Sx_manual = PropManualRuleEngine::finalPick3(
                        $race['propprearraywhorseno']['proppre'],
                        $race['propprearraywhorseno']['horseno'],
                        $manualRulesSx,
                        'Sx',
                        3
                    );
                    ...
                    $pageContent .= "<span style='font-size:11px;color:#666'> [ST/HV=auto · Sx=manual({$manualRulesSxN})"
                        . (count($preds_picks_Sx_manual) ? ' picks:' . implode(',', array_map(function ($p) { return $p['horseno']; }, $preds_picks_Sx_manual)) : ' no-pick')
                        . "]</span>";
                    if($ismm){
                        $pageContent .= "<br>---";
                    ...
                    foreach ($filteredHorses as $horseno => $horsedetail) {
                        ...
                        if (is_numeric($horseno) && is_array($horsedetail) && isset($horsedetail['name_ch'])) {
                            ...
                            foreach ($venues as $vkey) {
                                ...
                                if ($vkey === 'Sx') {
                                    // Manual rules platform (enabled Sx rules only)
                                    $$pickVar = $preds_picks_Sx_manual;
                                } else {
                                    // Auto-trained rules (unchanged for compare)
