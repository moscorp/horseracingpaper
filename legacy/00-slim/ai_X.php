...
//--------------------------------------------------------------------		
// https://bet.hkjc.com/racing/getJSON.aspx?type=winplaodds&date=2023-03-15&venue=hv&start=1&end=9
...
$today = date("Y-m-d");
$url = isset($_GET['venue']) ? "https://bet.hkjc.com/racing/script/rsdata.js?lang=ch&date=".$today."&venue=".strtoupper($_GET['venue'])."&CV=FO_L4.01R0f" : $url;
// echo $url;
...
foreach ($racedata as $raceno => $value) {
	if($raceno){
		$going = isset($value["going"]) ? $value["going"] : "";
		$racedatahead[$raceno] = $value["race"].", ".$value["venue"].", ".$value["time"].",  "
								.(isset($value["class"]) ? $value["class"] : "").", ".$value["track"].", ".$value["dist"].", ".$going;
...
for ($raceno = $start; $raceno <= $end; $raceno++) {
	// $windata = array();
	// sleep(1);
	$url = "http://bet.hkjc.com/racing/getJSON.aspx?type=winplaodds&date=".$racingdate."&venue=".$venue."&raceno=".$raceno;
	// echo $url."<br>";
// while(!$windata){
...
// http://bet.hkjc.com/racing/getJSON.aspx?type=qpl&date=2018-07-01&venue=ST&raceno=1
for ($raceno = $start; $raceno <= $end; $raceno++) {
	$url = "https://bet.hkjc.com/racing/getJSON.aspx?type=qin&date=".$racingdate."&venue=".$venue."&raceno=".$raceno;
	$qin[$raceno] = postRequest($url,[]);
	// print_r($qin);
	// echo "<br><br><br>";
}
for ($raceno = $start; $raceno <= $end; $raceno++) {
	$url = "https://bet.hkjc.com/racing/getJSON.aspx?type=qinpre&date=".$racingdate."&venue=".$venue."&raceno=".$raceno;
	$qinpre[$raceno] = postRequest($url,[]);
	// print_r($qin);
	// echo "<br><br><br>";
}
for ($raceno = $start; $raceno <= $end; $raceno++) {
	$url = "https://bet.hkjc.com/racing/getJSON.aspx?type=qpl&date=".$racingdate."&venue=".$venue."&raceno=".$raceno;
...
// 	die(1);
	...
	$update_RaceCard = "";
	for ($raceno = $start; $raceno <= $end; $raceno++) {
		if(substr($venue,0,2)=="S1" || $venue == "S2" || $venue == "S3" || $venue == "S4"){
			...
			// $RaceCard_url = "https://bet.hkjc.com/racing/pages/odds_wp.aspx?lang=ch&date=".$racingdate."&venue=".$venue."&raceno=".$raceno;
			// $RaceCard_url = "https://bet.hkjc.com/racing/pages/odds_wp.aspx?lang=ch&date=2022-07-27&venue=S2&raceno=1
			$RaceCard_url = "https://racing.hkjc.com/racing/SystemDataPage/racing/overseas/RaceCard-SystemDataPage.aspx?match_id=".str_replace("-","",$racingdate)."/".$venue."/".$raceno."&lang=Chinese";
		}else{
	...
	foreach($rowData as $raceno => $data){
		foreach($data as $row => $rowdetail){
			...
			if(substr($venue,0,2)=="S1" || $venue == "S2"){
...
$update_RaceCard = "";
for ($raceno = $start; $raceno <= $end; $raceno++) {
	$tri20_url = "https://bet.hkjc.com/racing/pages/odds_tri.aspx?lang=ch&date=".$racingdate."&venue=".$venue."&raceno=".$raceno;
	// echo $tri20_url."<br>";
...
// https://bet.hkjc.com/racing/pages/results.aspx?lang=ch&date=2023-08-13&venue=S1&raceno=6
...
// print_r( $htmls->find('span[class=f_fl f_fs13]'));
foreach($htmls->find('div[class=raceMeeting_select]') as $dispdate){
	foreach($htmls->find('p[class=f_clear]') as $group){
		// get tweet text
		$tweetText = $group->find('span.f_fl f_fs13', 0)->innertext;
...
if(!empty($result)){
	foreach($result as $raceno => $data){
		...
		// echo $result[$raceno][14];
		if(substr($venue,0,2)=="S1" || $venue == "S2" || $venue == "S3" || $venue == "S4"){
			$place = explode(",", $horseno);
			$place = explode(",", $result[$raceno][14]);
			$resultx[$raceno][$place[0]] = 1;
			$resultx[$raceno][$place[1]] = 2;
			$resultx[$raceno][$place[2]] = 3;
			$resultx[$raceno][$place[3]] = 4;
...
foreach($odds_sorted as $raceno => $racearray){
	...
	foreach($racearray as $horseno => $detail){
		if($raceno && is_numeric($horseno) && $racecard[$raceno][$horseno]['horsename'] && $poolStatus[$raceno]){
			...
			$isbestime = ($bestime_min[$raceno] == $racecard[$raceno][$horseno]['BestTime_val']) ? "<font color=red><b>" : "";
			...
			$TJratio_disp = (isset($TJratio[$racecard[$raceno][$horseno]['Trainer']][$jockeyshort[0]]['night_ratio']))
								...
								: "-";
			$TJratio_disp .= "</font>[".$TJratio[$racecard[$raceno][$horseno]['Trainer']][$jockeyshort[0]]['night_racecnt']."]";
			$TJratio_disp .= "/";
			$tjcss = ($TJratio[$racecard[$raceno][$horseno]['Trainer']][$jockeyshort[0]]['day_ratio']>=50) ? "<font color=red>" : "";
			$TJratio_disp .= (isset($TJratio[$racecard[$raceno][$horseno]['Trainer']][$jockeyshort[0]]['day_ratio']))
								? $tjcss.$TJratio[$racecard[$raceno][$horseno]['Trainer']][$jockeyshort[0]]['day_ratio']
								: "-";
			$TJratio_disp .= "</font>[".$TJratio[$racecard[$raceno][$horseno]['Trainer']][$jockeyshort[0]]['day_racecnt']."]";
...
// echo $detail['pre']["prop"]."-".$history_array[$raceno][$horseno]["max_prop_curr"]."-".$detail['pre']["prop"]."-".$history_array[$raceno][$horseno]["min_prop_curr"]."<br>";
			//-----------------------------------------------------------------------------------------------------------------------------------------
			$listr .= ($dispfull ? "<td $is_samewp_pre>".$is_samewp_pre3.$is_maxwp_pre.$firstbig3x.$is_24xontop.$detail['pre']["prop"] : "");
			//--
			...
			$listr .= ($dispfull ? "<td $is_samewp_curr>".$is_bigger_prop_pre.$is_maxwp_curr.$detail['curr']["prop"] : "");
			...
			$listr .= "<td>".$isbestime.$firstbig3x.$horseno;
			$listr .= "<td><b>".($resultx[$raceno][$horseno] ? $resultx[$raceno][$horseno] : "");
			...
			$listr .= "<td style='' $nosamehist>".(!$terminated ? "<input type=\"checkbox\" name=\"test\" value=\"value1\">" : "");
			$listr .= "<td style='' $oddchange>".(!$terminated ? "<input type=\"checkbox\" name=\"test\" value=\"value1\">" : "");			
			$listr .= "<td style='' $uppro >".(!$terminated ? "<input type=\"checkbox\" name=\"$checkboxname\" value=\"value1\" onchange=\"getCheckedBoxes('$checkboxname');this.checked^=1;\">" : "");
			...
			$listr .= "<td nowrap>".$islastjockey.$racecard[$raceno][$horseno]['lastodds'];
			list($winodds, $pay, $runtype) = explode('[', $racecard[$raceno][$horseno]['samejockey']);
			$listr .= "<td nowrap>".$dist.$winodds;	//.$racecard[$raceno][$horseno]['Dist'].$racedist[$raceno];
			$listr .= "<td nowrap>".$dist.$racecard[$raceno][$horseno]['runningtype'];
			
			$listr .= "<td nowrap >".$TJratio[$racecard[$raceno][$horseno]['Trainer']][$jockeyshort[0]]['total_ratio'].$TJdorn;
			$listr .= "<td nowrap $TJratio_total $TJratio_35_50>".$TJratio_disp;
			//--------------------------------------
			...
			$listr .= "<td nowrap bgcolor=#CDFF99><font color=brown>".$racecard[$raceno][$horseno]['Draw']*1;
			$listr .= "<td nowrap>".$accu_result[$racecard[$raceno][$horseno]['Draw']];
			...
			$listr .= "<td nowrap>".$accu_result[(string)$racecard[$raceno][$horseno]['Trainer'].(string)$racecard[$raceno][$horseno]['Jockey']];
			$listr .= "<td nowrap>".$racecard['Trainer'][(string)$racecard[$raceno][$horseno]['Trainer']];
			$listr .= "<td nowrap>".$accu_result[(string)$racecard[$raceno][$horseno]['Trainer']];
			$listr .= "<td nowrap>".$racecard[$raceno][$horseno]['Trainer'];
			$listr .= "<td nowrap>".$racecard['Jockey'][(string)$racecard[$raceno][$horseno]['Jockey']];
			$listr .= "<td nowrap>".$accu_result[(string)$racecard[$raceno][$horseno]['Jockey']];
